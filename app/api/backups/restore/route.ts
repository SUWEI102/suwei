...
import { getTencentSnapshot } from '@/lib/tencent-backup'
...
export async function POST(request: Request) {
  try {
    ...
    const body = await request.json() as { mode?: 'preview' | 'restore'; payload?: unknown; confirmation?: string; source?: 'upload' | 'tencent'; snapshotId?: number }
    const rawPayload = body.source === 'tencent' && Number.isInteger(body.snapshotId)
      ? await getTencentSnapshot(userId, body.snapshotId as number)
      : body.payload
